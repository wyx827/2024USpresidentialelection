...
# Set column names for coefficients_df
colnames(coefficients_df) <- c("Estimate", "Std. Error", "t value")[1:ncol(coefficients_df)]
...
additional_stats <- data.frame(
  Estimate = round(c(r_squared, adj_r_squared, f_statistic, residual_se), 2),
  `Std. Error` = NA,
  `t value` = NA,
  row.names = c("R-squared", "Adjusted R-squared", "F-statistic", "Residual Std. Error")
)

# Ensure column names are identical between data frames
additional_stats <- additional_stats[, colnames(coefficients_df), drop = FALSE]
