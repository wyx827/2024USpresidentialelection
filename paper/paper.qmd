...
subtitle: "Poll Data Predicts Kamala Harris Win with 387 Electoral Votes"
author: 
  - Xuanle Zhou
  - Yongqi Liu
  - Yuxuan Wei
...
## Estimand
In this analysis, the estimand is the true effect of polling factors on the projected support for Trump and Harris in the 2024 U.S. election. Specifically, this effect encompasses how variables like poll quality, transparency, sample size, and timing influence predicted vote percentages for Kamala Harris and Donald Trump across states. This build-up of estimand ensures consistency by clarifying that our target effect is the overall impact of these factors on candidates’ state-by-state electoral prospects. The estimand guides the model design and interpretation to achieve our goal of predicting the election outcome.
...
This study uses R packages [@citeR] to clean and analyz the dataset, including libraries from tidyverse [@tidyverse], ggplot2[@ggplot2], knitr[@knitr],usmap[@usmap], corrplot[@corrplot], and arrow [@arrow].
...
The outcome variable of interest for this research is the percentage representing the level of public support for Donald Trump. The distribution is shown in @fig-pct, where the x-axis represents the support percentage, ranging from approximately 20% to 70%, and the y-axis displays the count of poll surveys reporting each support level. The distribution reveals that most observations cluster around a support percentage of approximately 48%, indicating a substantial level of backing from the electorate. Additionally, a smaller proportion of polls show support exceeding 55%, suggesting that while Trump has a core base, many voters remain either indifferent or opposed to him. This visualization provides insight into both the concentration and variability of support within the surveyed population.

```{r}
#| label: fig-pct
#| fig-cap: Distribution of Support for Donald Trump in Percentage Across Poll Survey Responses
#| echo: false
#| warning: false
#| message: false
...
ggplot(analysis_data %>% 
  ...
  labs(x = "Transparency Score of Poll", 
       y = "Count of Distinct Pollsters with Each Transparency Score") + 
  theme_minimal()
```
...
#| tbl-cap: Regression Model Summary Table（Display the first 10 rows）
#| warning: false
#| message: false
...
coefficients_table <- summary$coefficients[, 1:3]
...
The table in the [Appendix -@tbl-missing-states] serves as a supplement to the predicted electoral outcomes by state, filling in the gaps for states where the model couldn’t provide predictions. It allows for a complete view of electoral projections by assigning a likely outcome in each missing state based on historical data. This approach ensures every state is accounted for in the final electoral vote totals.
...
## Weakness
Despite the model estimating the voter's choice trend, it has limitations. For instance, assuming linear relationships between predictors may oversimplify complex voter behaviors and other external social influences. The dataset simplifies the potential influence behind the US election. 
...
Due to limited data available for predicting outcomes in every state, some state projections rely on historical voting patterns, with the assumption that these trends will persist. However, this approach may be challenged by the rapidly changing political landscape in the U.S., where evolving demographics and shifting public sentiment could lead to unexpected results.
...
Other approaches that SurveyUSA could consider include imputation, as discussed in paper published by @bethlehem2010selection. This method estimates missing responses based on available data, helping to fill gaps and reduce bias in cases of partial non-response.
...
The primary goal of this survey methodology is to forecast the outcome of the U.S. presidential election by collecting high-quality, representative data from a diverse cross-section of American voters. Given a budget of $100,000, this methodology leverages sophisticated sampling techniques, effective respondent recruitment strategies, and robust data validation protocols to ensure maximum accuracy and minimal bias. This approach is designed to account for demographic, geographic, and political factors that can influence voting behavior, thereby enhancing the reliability of predictions [@fivethirty; @pewresearch].

## Core Objectives
* Obtain a representative sample of the U.S. electorate, with demographic diversity mirroring national voting patterns.
* Ensure data quality through rigorous validation processes, including screening and cross-verification.
* Utilize statistical modeling and poll aggregation techniques to provide accurate predictions, as outlined in prior studies of electoral polling [@silver_nytimes].

## Sampling Strategy
Our sampling strategy combines **stratified random sampling** with **quota sampling** to ensure broad representation across key demographics. This method has been shown to improve the accuracy of electoral predictions by reducing demographic and geographic sampling bias [@fivethirty].
...
This stratification aligns with best practices in survey sampling, allowing for improved accuracy and generalizability in election forecasting [@pewresearch].
...
Recruitment is executed through  **multi-channel outreach**, combining digital advertisements, email outreach, and partnerships with civic organizations. This diverse recruitment strategy helps to capture a representative sample and has been identified as a best practice in survey methodologies [@pewresearch].
...
These practices align with standards for data integrity and quality control in electoral polling [@fivethirty; @silver_nytimes].

## Poll Aggregation and Data Analysis
### Poll Aggregation
This survey will be combined with results from reputable polling firms (e.g., YouGov, Ipsos, Gallup) to strengthen the forecast through a **poll-of-polls** approach [@silver_nytimes].
...
kable(full_summary_df[!rownames(full_summary_df) %in% "F-statistic p-value", c("Estimate", "Std. Error", "t value")])
```
...
This project utilized R @citeR and a variety of R packages that provided essential tools and functionality. We thank the @tidyverse, @ggplot2 and @knitr teams for their extensive suite of tools for data manipulation and visualization, which were instrumental in data cleaning and graphing. We also acknowledge @usmap for its support in creating U.S. geographic visualizations, and @corrplot for facilitating clear, informative correlation plots. Finally, @arrow was crucial for efficient data storage and handling with Parquet files. We are grateful to the developers and maintainers of these packages for their contributions to open-source software.
