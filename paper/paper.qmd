...
Here we briefly describe the logistic regression model used to investigate the winning probability of Trump. Background details and diagnostics are included in [Appendix -@sec-model-details].
...
By incorporating state and demographic information along with pollster quality and methodology, we aim to capture the variability in polling predictions across different regions and polling practices. The use of Bayesian inference allows us to quantify uncertainty in our predictions more effectively.
...
set.seed(853)
analysis_data_reduced<- analysis_data_reduced %>% na.omit(analysis_data_reduced)
# Split the data (70% training, 30% testing)
trainIndex <- createDataPartition(analysis_data_reduced$trump_win, p = 0.7, list=FALSE)
train_data <- analysis_data_reduced[trainIndex, ]
test_data <- analysis_data_reduced[-trainIndex, ]
...
# Binarize the predicted probabilities using a threshold of 0.5
predicted_classes <- ifelse(predicted_probabilities > 0.5, 1, 0)

# Confusion Matrix
conf_matrix <- confusionMatrix(factor(predicted_classes), factor(test_data$trump_win))
print(conf_matrix)

# Precision, Recall, and F1-score
precision <- posPredValue(factor(predicted_classes), factor(test_data$trump_win))
recall <- sensitivity(factor(predicted_classes), factor(test_data$trump_win))
...
```{r}
# Load the pre-trained logistic model from RDS
logistic_model <- readRDS(here:here("models/logistic_model.rds"))

# Calculate Trump and Harris percentages for each state (you still need to prepare the data)
...
# Create 10-fold cross-validation resampling
folds <- vfold_cv(analysis_data, v = 10)

# Fit the resamples using the loaded logistic model
cross_validation <- fit_resamples(
  logistic_model,
  data = analysis_data,
  resamples = folds,
...
collect_metrics(cross_validation) %>%
  select(.metric, mean) %>%
  knitr::kable(
    col.names = c("Metric", "Mean"),
    digits = 2,
    format.args = list(big.mark = ",")
  )

# Display confusion matrix for resampled results
conf_mat_resampled(cross_validation) %>%
