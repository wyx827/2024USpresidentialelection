...
The remainder of this paper is organized as follows: @sec-data describes the dataset and the preprocessing steps for model preparation. @sec-model explains the modeling approach, including predictor selection and the regression model structure. @sec-results presents the primary findings, detailing state-by-state predictions and overall electoral projections. @sec-discussion discusses the implications, limitations, and potential directions for future research. The Appendix provides supplementary information on methodology, diagnostics, technical details of the model setup, acknowledgments, and references.
...
This study uses R packages [@citeR] to clean and analyz the dataset, including libraries from tidyverse [@tidyverse], ggplot2[@ggplot2], knitr[@knitr],usmap[@usmap], corrplot[@corrplot], arrow [@arrow]. 
...
The dataset used in this analysis originates from @fivethirty, a trusted source known for its rigorous methodology in collecting and aggregating polling data to represent public opinion accurately. To maintain data integrity and ensure a high standard of reliability, only polls meeting stringent quality criteria are included in the dataset. Specifically, each poll must disclose important information, including the pollster’s name, the exact dates of the survey, the sample size, and the full methodological details. These details encompass the polling medium (e.g., phone, online), use of verified voter files, demographic weighting criteria, and any adjustments applied to reflect a representative sample.
...
Once verified as credible, polls are incorporated into the FiveThirtyEight database. These polls then contribute to polling averages, forecast models, and political analysis, all of which rely on high-quality, reliable public opinion data. This careful selection process ensures that the dataset reflects a thorough and detailed view of public sentiment, accurately capturing behavioral trends and preferences over time.
...
```{r}
#| label: fig-end-date
#| fig-cap: Trend of Support Percentage for Donald Trump Over Days Until Election. The red dashed line indicates the average support rate for Donald Trump at that point in time. Renew the data to the October 24.2024
#| echo: false
#| warning: false
#| message: false
...
- Florida: The model predicts a close race, with Trump at 48.89% of the vote, resulting in a Harris win in this battleground state.
...
The table in the [Appendix -@tbl-missing-states] supplements the predicted electoral outcomes by providing likely results for states where the model could not generate predictions. Using historical data to estimate outcomes for these missing states ensures a complete view of electoral projections and accounts for every state in the final electoral vote totals.
...
The primary goal of this survey methodology is to forecast the U.S. presidential election outcome by collecting representative data from a diverse cross-section of American voters. With a $100,000 budget, this methodology employs sampling techniques, effective recruitment strategies, and data validation protocols to ensure accuracy and minimize bias. This approach is designed to account for demographic, geographic, and political factors that can influence voting behavior, thereby enhancing the reliability of predictions [@fivethirty; @pewresearch].

## Core Objectives
* Obtain a representative sample of the U.S. electorate, with demographic diversity mirroring national voting patterns.
* Ensure data quality through validation processes, including screening and cross-verification.
...
## Sampling Strategy
Our sampling strategy combines **stratified random sampling** with **quota sampling** to ensure broad representation across key demographics. This method improves electoral prediction accuracy by reducing demographic and geographic sampling bias [@fivethirty].
...
Recruitment is executed through  **multi-channel outreach**, combining digital advertisements, email outreach, and partnerships with civic organizations. This recruitment strategy helps capture a representative sample and aligns with best practices in survey methodologies [@pewresearch].
...
Maintaining data integrity and ensuring high-quality responses are important to the accuracy of the election forecast. Several measures will be put in place to validate responses and reduce noise in the dataset.
...
These practices align with data integrity and quality control standards in electoral polling [@fivethirty; @pew_survey_methodology].
